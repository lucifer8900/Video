...
- **CX-514 人物动态表演提示词重构与救援首帧 v3 候选**：根据人工对现有生成视频的逐帧审查，直接重构 CX-505 原文件中的 10 条主剧情和 30 条 NPC 回应无声视频提示词，不新增提示词覆盖文件。连续性约束只锁定人物身份、面容、服装、道具身份/数量和场景拓扑，不再锁死姿势、手部、屏幕坐标或要求末帧返回首帧；每条提示词必须声明可见的人物表演节拍、自然呼吸与重心变化、镜头可读性和手部完整性，同时保留闭口、无伪口型、无字幕和无对白音轨要求。不得修改剧情事实、台词、人物/场景名称、状态变化、媒体 ID 或世界规则。使用 Codex 内置 ImageGen 为 `node.rescue` 生成版本化 `firstframe_node_rescue_grand_v3` 候选，以既有沈砚、楚明绮身份锚点和 v2 救援场景作参考，采用自然低位手势、清晰五指/手腕、双方处于共同景深且全程可保留的构图；输出到 `content/visual-candidates/cx514/`，严格 1664×936 PNG，记录完整提示词、输入角色、尺寸、字节数和 SHA-256，并生成 v2/v3 联络表。候选保持 `needs_review`、`adopted=false`、`shipInBuild=false`，不得覆盖 v2、不得进入 Unity `Resources`/`StreamingAssets`/Addressables/构建，也不得提前改写 CX-505 的上传路径；人工审核通过后的采用与路径切换另开任务卡。不得调用 Gemini、Veo 或任何视频生成服务。
  - 验收：先以失败测试证明缺少 CX-514 Schema/清单/验证命令、提示词数量不是 10+30、任一提示词仍锁死人物姿势/手部/屏幕坐标或要求末帧复位、缺少可见表演节拍/人物始终可读/手部完整性/闭口无字幕约束，以及候选采用、进入 Unity、覆盖 v2、非版本化文件名、非 PNG、非 1664×936、文件长度或 SHA-256 漂移都会被拒；随后 CX-505 原文件的 40 条提示词全部通过结构与业务校验，`rescue` 明确禁止主角被推镜裁出或消失，v3 候选及 v2/v3 联络表齐备且仍待人工审核。CX-514 专项测试、Schema 测试、ReferenceLibrary 全量测试、`Video.sln` 全量测试和 Release 构建保持全绿；报告真实输出并确认未调用 Gemini/Veo、未生成视频。

- **CX-515 Flow2API 回环隧道视频 Provider 与单次 Lite 烟雾生成**：暂停未确认的 CX-514，根据人工明确提供的 Flow2API 服务与“只测试生成一个视频”授权，在服务端现有 `IVideoGenerationProvider` 边界内新增 OpenAI-compatible Flow2API Provider，固定向 `http://127.0.0.1:18001/v1/chat/completions` 发出 `POST`，使用 `Authorization: Bearer <credential>`、`Content-Type: application/json` 和 `stream=true`，解析 SSE 进度与最终视频 URL；横屏 8 秒默认模型固定为 `veo_3_1_t2v_lite_landscape`（无图片）和 `veo_3_1_i2v_lite_landscape`（单首帧），不得静默升级到 Fast/标准/4K 模型。凭据只从用户级环境变量 `Flow2API` 读取，不把值写入仓库、配置、命令输出、异常或日志。HTTP 只允许精确回环地址与端口的 SSH 隧道例外；禁止公网 IP、非回环主机、用户信息、查询串和重定向，且绝不访问此前提供的公网地址。下载 URL 只允许同一回环来源或经显式白名单批准的 HTTPS 来源，并经过大小、MP4 签名与 SHA-256 校验。Git 内默认仍保持 `ManualPrompt`/Mock 及网络关闭；本机被 Git 忽略的覆盖配置可把 Flow2API 设为默认视频 Provider，但必须同时启用预算门禁。实现只用 Mock HTTP/SSE 做自动测试，不在单测访问真实服务。
  - 验收：先以失败测试证明未知模式、任意公网/非回环 HTTP、错误端口或路径、带凭据 URL、缺失的 `Flow2API` 环境变量、错误 Lite 模型、非 POST、缺 Bearer、`stream` 非 true、非 SSE/畸形 SSE、错误/重复完成事件、越权下载、超限媒体、密钥泄漏以及客户端侧配置都会被拒；随后 Mock 覆盖 T2V、单首帧 I2V、进度、失败、取消和安全下载，完整解决方案保持全绿。真实烟雾测试必须在确认回环隧道可用、模型列表正确且预算门禁生效后才执行，客户端不自动重试，只提交一次 16:9、8 秒、1 个结果、无声 I2V 请求，使用既有已采用首帧与非发货测试提示词；产物保存到 Git 忽略的 `F:/VideoGame/TestVideo/`，登记请求/响应脱敏摘要、MP4 长度和 SHA-256，不登记密钥，不自动采用到游戏。完成后报告真实调用次数、模型 ID、耗时与文件校验；未经新的人工指令不得继续批量生成。
  - 人工诊断例外（2026-07-28 至 2026-08-01）：回环隧道首轮失败后，用户逐次明确授权了额外的一次性重试和指定公网地址诊断；每次均设置独立审计锁、`stream=true`、`automaticRetries=0`，凭据值与签名下载 URL 未写入仓库或报告。该例外不改变正式 Provider 的精确回环端点、Git 内 `ManualPrompt`/网络关闭默认值或公网端点拒绝测试。最终成功响应证明 Flow2API 返回顶层 `url`、`generated_assets.final_video_url` 或聊天内容内嵌结构化 JSON，实际下载地址为无文件扩展名的 `https://flow-content.google/video/<UUID>` 签名 URL；Provider 因此只对白名单主机、UUID 路径以及恰好包含 `Expires`、固定 `KeyName`、合法 `Signature` 的 HTTPS 地址开放下载，继续拒绝 HTTP、仿冒域名、错误路径、多余查询参数、用户信息、片段和重定向。

- **CX-516 仙侠服装、建筑与景观研究及原创美术方向圣经**：根据人工批准，在恢复 CX-514 前先为《赤雾秘苑》现有六个人形角色（沈砚、楚明绮、石峻、青衣侦察者、洞窟盟友、阵灵）与一只墨蛟建立新的研究和设计基线。主方向固定为“唐风宏大仪制＋宋式清雅日常＋原创仙侠”：以可建造的宫阙、山门、园林、药苑、洞府、阵坛、云海和天外奇观承载宏大尺度，以干净合体的交领、圆领、窄袖、广袖、披帛、腰封、织锦、绫罗、玉石和鎏金金属建立可信服装层次；禁止大面积脏灰、无剧情依据的破布/磨损、现代皮鞋、清朝服饰、塑料感盔甲、演员相似、具体影视角色服装/纹样/布景一比一复刻和不同作品直接拼贴。研究清单只登记官方页面、作品/平台、观察类别和抽象设计结论；任何下载剧照只允许留在 Git 忽略的私人参考目录，`referenceOnly=true`、`shipInBuild=false`、`identityReuse=false`，不得进入 Unity 或发货包。服装矩阵固定为沈砚与楚明绮各 3 套（前/中/后期）、石峻与青衣侦察者各 2 套、洞窟盟友与阵灵各 1 套，共 12 套；墨蛟只建立原创中国蛟龙形态、鳞色、角须、四爪和尺度规范，不做人类服装。环境规范必须覆盖仙门宫阙、园林药苑、山水云海、洞府地宫、阵法祭坛、战斗空间、天气天象和天外奇观，并为日景、暮景、夜景建立明暗对比而非全局压暗。本卡只新增 `red-mist-art-direction.cx516` 机器清单、可读说明、Draft 2020-12 Schema、CLI 业务校验和契约测试；不生成 ImageGen 图片、不调用 Flow2API/Veo/Gemini、不改剧情/台词/人物名称、不覆盖 CX-508~513 资产、不采用进 Unity，CX-514 继续暂停。
  - 验收：先以失败测试证明缺少 CX-516 Schema、CLI 命令或清单时红灯；随后校验必须拒绝少于 8 个官方作品来源、缺服装/建筑/景观/光影任一观察维度、演员身份复用、精确复刻、可发货/运行时路径、错误主美术方向、角色顺序或服装总数/阶段数量不符、墨蛟被当作人类服装、缺任一环境域、允许脏灰破布/现代鞋/清朝服饰、全局黑暗、生成图片/视频服务调用或提前恢复 CX-514。机器清单与 Markdown 必须逐项对应，记录人工批准日期与六人一蛟范围；CX-516 专项测试、Schema/ReferenceLibrary 全量、`Video.sln` 全量和 Release 构建保持全绿。完成后另开人物多视图卡，使用内置 GPT Image 2 逐资产生成并人工审核。

- **CX-517 六人十二套服装与墨蛟多视图候选**：继承人工批准的 CX-516，不恢复 CX-514；使用 Codex 内置 GPT Image 2 / `imagegen`，以现有沈砚 v1、楚明绮 v2、石峻 v3、青衣侦察者 v3、洞窟盟友 v1、阵灵 v2 和墨蛟 v3 作为项目内参考锚点，逐资产生成 12 张人物服装三视图候选和 1 张墨蛟多视图候选。每张人物板固定同一原创身份、同一发型、同一服装结构/配色/材质/鞋履/连续性道具，画面从左到右依次为正面、左侧面、背面三个完整全身视图；使用相同中性站姿、自然七头半左右比例、标准长焦透视、暖象牙无缝摄影棚背景、柔和全局日光与接触阴影，不添加文字、标签、场景人物或装饰。三个人像必须是同一角色同一套衣服的转身记录，不得出现三张不同脸、不同年龄、不同服装、不同道具或不同人体比例。墨蛟板固定同一头部、双角、须鬃、鳞色、蛇形长躯、四肢四爪和无翼结构，展示正侧背/俯视关系并保留完整头尾，不得退化为蜥蜴或西方龙。所有输出使用版本化 PNG 文件名并保存到 `content/visual-candidates/cx517/turnarounds/`；完整记录 CX-516 SHA-256、锚点路径/哈希、规范化提示词、生成模式、尺寸、字节数、SHA-256、三视图顺序、道具锁和人工审核项，保持 `needs_review`、`adopted=false`、`shipInBuild=false`，不得写入 Unity、覆盖 CX-508~513 或被任何运行时引用。72 张人物表情参考必须等本卡身份/服装人工通过后另开任务卡，避免把未批准的脸部漂移复制到全部情绪资产；不得调用 Flow2API、Gemini、Veo 或生成视频/音频。
  - 验收：先以失败测试证明缺少 CX-517 Schema、CLI 命令或清单时红灯；随后校验必须拒绝 CX-516/锚点路径或哈希漂移、角色/服装顺序不是 12 项、漏掉正/侧/背任一视图、提示词未锁定同脸/同发型/同人体/同服装/同道具、出现文字或额外人物、错误服装阶段/数量、墨蛟缺双角须鬃/四肢四爪/无翼/反蜥蜴约束、采用/可发货、Unity 运行时路径、非版本化文件名、非 PNG、文件不存在、签名/长度/尺寸/SHA-256 不符、ImageGen 以外生成模式或 CX-514 被恢复。随后 13 张候选全部存在且机器清单与 Markdown 逐项对应；每项保留人工待审的身份一致、三视图一致、比例、手脚、服装结构、材质、鞋履、道具、无文字水印和非演员相似检查。本卡只产出技术设计候选，尚不进入 Unity；CX-517 专项、Schema/ReferenceLibrary 全量、`Video.sln` 全量和 Release 构建保持全绿，并报告真实 ImageGen 调用次数与未调用的视频服务。

- **CX-518 六人十二套服装与墨蛟多视图采用**：依据用户在 2026-08-03 对 CX-517 十三张候选作出的“全部采用”明确决定，建立不可变采用记录；将 12 张人物服装三视图和 1 张墨蛟多视图从 `needs_review` 批准为后续美术生成的正式参考锚点，但不回写或改造 CX-517 provenance。采用含义限定为 `adoptedAsReference=true`、`expressionGenerationAllowed=true`；三/四视图技术板不是游戏画面，不复制到 Unity、不创建 Resources 路径、`shipInBuild=false`、`runtimeUse=false`。机器记录必须锁定 CX-517 清单路径/哈希、13 个有序资产 ID、源文件路径/尺寸/字节数/SHA-256、人工批准日期与决定，保留所有旧身份锚点和历史候选；六名人物各 12 种表情共 72 张可在本卡完成后另开任务卡生成，CX-514 继续暂停。不得生成新图片、视频或音频，不调用 ImageGen、Flow2API、Gemini 或 Veo，也不得修改剧情、台词、人物名称和 Unity 内容。
  - 验收：先以失败测试证明缺少 CX-518 Schema、CLI 命令或采用清单时红灯；随后校验必须拒绝人工批准缺失、资产不是完整有序 13 项、CX-517 清单/源文件路径或 SHA-256 漂移、尺寸/长度/PNG 签名不符、源候选 provenance 被改成 adopted、任一资产未采用作参考、允许进入 Unity/运行时/发货、覆盖旧资产、表情仍被禁止或 CX-514 被恢复，以及任何图像/视频/音频服务调用声明。机器清单与 Markdown 必须逐项对应，CX-518 专项、Schema/ReferenceLibrary 全量、`Video.sln` 全量和 Release 构建保持全绿；完成后以新任务卡开始 72 张表情参考资产。

- **CX-519 六名人物十二表情参考资产**：继承 CX-518 已明确采用的参考锚点，为沈砚、楚明绮、石峻、青衣侦察者、洞窟盟友、阵灵六名人物分别生成 `neutral`、`calm_warmth`、`restrained_joy`、`resolve`、`vigilance`、`suspicion`、`concern`、`grief`、`anger_controlled`、`fear_suppressed`、`pain`、`astonishment` 十二种表情，共 72 张独立 PNG。使用 Codex 内置 GPT Image 2 / `imagegen`，每名人物以获准身份锚点、CX-517 对应服装锚点和上一组通过的表情板共同锁定脸型、年龄、发型、领口、正面 85mm 机位、头肩裁切、暖象牙高调棚拍光线；每四种表情先生成一张 2×2 母板，A/B/C 三组共 18 张，再以确定性裁切和 Lanczos 放大形成 72 张 1536×1024 独立参考图。表情只改变面部肌肉，不得改变身份、年龄、妆容、发型、服装、机位或光照；禁止文字、标签、水印、手、道具、额外人物、演员相似、畸形眼睛、蜡质皮肤、夸张哭喊、伤口与身份漂移。所有母板和独立图均使用版本化文件名，记录 CX-518 清单路径/SHA-256、身份与服装锚点、规范化提示词、实际 ImageGen 调用数、母板/象限来源、PNG 尺寸/长度/SHA-256 与人工审核项；保持 `needs_review`、`adopted=false`、`runtimeUse=false`、`shipInBuild=false`，不得写入 Unity。CX-514 继续暂停，不调用 Flow2API、Gemini、Veo，不生成视频或音频。
  - 验收：先以失败测试证明缺少 CX-519 Schema、CLI 命令或清单时红灯；随后校验必须拒绝 CX-518 清单路径/SHA-256 漂移或未解锁表情生成、人物不是固定有序 6 名、表情不是固定有序 12 种、总数不是 72、母板不是 A/B/C 三组或每组象限映射错误、身份/服装锚点路径或哈希漂移、提示词未锁定身份/机位/光线或缺少禁止项、文件未版本化、不是 PNG、文件不存在、PNG 签名/尺寸/长度/SHA-256 不符、独立图来源母板/象限不符、采用/运行时/发货、覆盖旧资产、非内置 ImageGen、生成视频音频或恢复 CX-514。机器清单与 Markdown 必须列出全部 18 张母板和 72 张独立图；每张独立图保留身份一致、表情可读、双眼解剖、皮肤质感、领口一致、无文字水印和非演员相似人工审核项。本卡只产出待审技术参考，不进入 Unity；CX-519 专项、Schema/ReferenceLibrary 全量、`Video.sln` 全量和 Release 构建保持全绿，并报告实际 ImageGen 调用次数与未调用的视频服务。
...
| 2026-08-01 | M5 Flow2API 视频 Provider（CX-515 完成） | 新增只允许精确回环隧道的 Flow2API 视频 Provider、用户级 `Flow2API` 凭据读取、固定 Veo 3.1 Lite T2V/I2V 模型、单首帧 data URL、SSE、取消、大小/MP4/SHA-256 校验及 Mock 自动测试；Git 默认仍为 `ManualPrompt` 且网络关闭。人工逐次授权的诊断共发起 4 次独立客户端提交、自动重试始终为 0：首次在隧道监听消失时未到达服务，第二次回环与首次公网诊断收到 SSE 错误，Token 恢复后的第二次公网诊断成功完成唯一一次上游生成；服务端 `total_ms=68123`，结果解析因无扩展名签名 URL 首轮误判后，依据人工提供的成功响应增加 `flow-content.google/video/<UUID>` 严格白名单并用 GET 恢复同一产物，未新增生成。产物为 H.264 1280×720、24 fps、8.000 秒、`2,917,625` 字节、SHA-256 `7bbeb84d080043566b8f764cf1caaf2e6d26cab9a9d6c2cd3c163f8e23e32d9a`；首/中/末帧人物和场景连续，文件含 AAC 环境音（平均约 `-48.3 dB`、峰值 `-21.0 dB`），尚未采用进游戏。解析红灯 `0/2` 后新增成功/安全用例 `8/8`、CX-515 专项 `29/29`、Generation 全量 `505/505`、Video.sln 全量 `928` 通过且 `1` 个既有 FFmpeg 集成测试按默认门禁跳过，Release 构建 `0` 警告/`0` 错误；公网诊断未进入正式配置，后续批量生成、音轨处理和游戏采用仍须另卡及人工授权。 |
| 2026-08-03 | M5 原创美术方向圣经（CX-516 完成） | 根据人工批准，将《赤雾秘苑》范围固定为沈砚、楚明绮、石峻、青衣侦察者、洞窟盟友、阵灵六个人形角色与一只墨蛟，主方向固定为“唐风宏大仪制＋宋式清雅日常＋原创仙侠”。新增严格 Draft 2020-12 Schema、机器清单、可读美术圣经、CLI 业务校验与 20 项安全变异测试；研究清单记录 `9` 个官方平台作品页面及服装/建筑/景观/光影四类抽象结论，所有研究固定 `referenceOnly=true`、`shipInBuild=false`、`identityReuse=false`，禁止演员相似、具体服装/纹样/布景精确复刻。服装矩阵固定沈砚/楚明绮各 3 套、石峻/青衣侦察者各 2 套、洞窟盟友/阵灵各 1 套，共 `12` 套；每名人形角色固定 `12` 种表情参考资产；墨蛟固定为四肢四爪、无翼、蛇形长躯的原创中国蛟龙；环境覆盖 `8` 个可交互域及各自日/暮/夜光照。未来视频约束记录为 Veo 3.1 Lite、每批最多 5 段、每镜头 1 个、零自动重试、SSH 回环启用后才派发，并移除 Veo 自带声音、配音与音效独立制作；当前 `dispatchEnabled=false`，CX-514 继续暂停。本卡未生成图片或视频、未调用 ImageGen/Flow2API/Veo/Gemini、未改剧情/台词/名称、未覆盖或采用 Unity 资产。首轮红灯 `0/20`，最终 CX-516 `20/20`、ReferenceLibrary `156/156`、Schema `41/41`、Video.sln `948` 通过、`0` 失败、`1` 个既有 FFmpeg 集成测试按默认配置跳过，Release 构建 `0` 警告/`0` 错误。 |
| 2026-08-03 | M5 服装与生物多视图候选（CX-517 完成，待人工审核） | 继承 CX-516 主美术方向与已批准身份锚点，使用 Codex 内置 ImageGen 生成沈砚/楚明绮各 3 套、施峻/青瓷斥候各 2 套、洞窟盟友/阵灵各 1 套，共 `12` 张人物服装三视图，并生成 `1` 张墨蛟四视图候选；13 张均为 1536×1024、3:2、版本化 PNG。实际 ImageGen 调用 `15` 次，施峻首轮 2 张因阵钉退化为剑鞘式长物被淘汰，返工后改为掌长封闭短匣；最终保留 `13` 张，全部 `needs_review`、`adopted=false`、`shipInBuild=false`，未写入 Unity。新增严格 Draft 2020-12 Schema、机器清单、人工审核表、CLI 业务/锚点/PNG 签名/尺寸/哈希校验和 20 类不安全变异测试；墨蛟候选已恢复中国蛟龙头型、角须鬃、长躯和无翼特征，但侧视局部肢体被躯干遮挡，明确留作人工审核重点，不自动采用。六名人物各 12 种表情共 72 张继续延后，CX-514 保持暂停；未调用 Flow2API、Gemini、Veo，未生成视频或音频。首轮红灯 `0/23`，最终 CX-517 `23/23`、ReferenceLibrary `179/179`、Schema `41/41`、Video.sln `971` 通过、`0` 失败、`1` 个既有 FFmpeg 集成测试按默认配置跳过，Release 构建 `0` 警告/`0` 错误。 |
| 2026-08-03 | M5 多视图正式采用（CX-518 完成） | 用户明确回复“全部采用”，批准 CX-517 的 12 张人物服装三视图与 1 张墨蛟四视图。新增严格 Draft 2020-12 采用 Schema、机器采用清单、可读采用记录、CLI 业务/源清单/PNG 签名/尺寸/长度/哈希校验和安全变异测试；采用清单固定 CX-517 清单 SHA-256 与有序 13 项源资产，保留 CX-517 `needs_review`、`adopted=false`、`shipInBuild=false` 的不可变 provenance，不回写历史。采用语义限定为 `adoptedAsReference=true` 与 `expressionGenerationAllowed=true`，技术板仍为 `runtimeUse=false`、`shipInBuild=false`，未复制到 Unity、未创建 Resources 或运行时注册；六名人物各 12 种表情共 72 张现已获准在下一张任务卡生成，CX-514 继续暂停。本卡未生成图片/视频/音频，ImageGen、Flow2API、Gemini、Veo 调用均为 `0`。首轮红灯 `0/19`，最终 CX-518 `19/19`、ReferenceLibrary `198/198`、Schema `41/41`、Video.sln `990` 通过、`0` 失败、`1` 个既有 FFmpeg 集成测试按默认配置跳过，Release 构建 `0` 警告/`0` 错误。 |
| 2026-08-03 | M5 六名人物十二表情参考资产（CX-519 完成，待人工审核） | 继承 CX-518 明确采用的身份/服装锚点，为沈砚、楚明绮、石峻、青衣侦察者、洞窟盟友、阵灵生成固定有序 12 种表情。每人 A/B/C 三张 2×2 母板，共 `18` 张 1536×1024 PNG；再从每个理论象限内收分隔线，以 FFmpeg `756×504` 确定性裁切并用 Lanczos 放大到 1536×1024，形成 `72` 张独立 PNG。使用 Codex 内置 GPT Image 2 / `imagegen`，实际调用 `24` 次：`19` 次产生图像、`5` 次网络失败或挂起无产出；其中 `1` 张方形洞窟盟友 C 组候选因画布不合格被淘汰，最终保留 18 张母板。新增严格 Draft 2020-12 Schema、带 CX-518/身份/服装锚点哈希与母板/象限/裁切/PNG 长度尺寸哈希的机器清单、可读人工审核记录、可复现清单生成脚本、CLI 业务校验与 13 类安全变异测试；全部资产保持 `needs_review`、`adopted=false`、`runtimeUse=false`、`shipInBuild=false`，未写入 Unity。CX-514 继续暂停；Flow2API、Gemini、Veo、视频和音频调用均为 `0`。首轮红灯 `0/17`，实现中间状态 `16/17` 后修正 Markdown 转义与提示词契约，最终 CX-519 `17/17`、ReferenceLibrary `215/215`、Schema `41/41`、Video.sln `1007` 通过、`0` 失败、`1` 个既有 FFmpeg 集成测试按默认配置跳过，Release 构建 `0` 警告/`0` 错误。 |
