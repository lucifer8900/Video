...
| 2026-07-16 | M3 开发完成（受环境门禁） | CX-301~306 完成 API、任务状态机、Provider、转码分发、成本控制与 Unity 生成闭环；Mock/非集成测试全绿，真实云调用保持关闭，PostgreSQL 实测仍需显式测试连接串 |
| 2026-07-16 | M4 进行中 | CX-401~405 已完成 Ledger、AssociationTemplate、关联决策服务、一致性守卫及 Unity StoryThread 织入；CX-406 待完成。CX-405 使用人工批准的 `prologue/node_intro`、`flight/travel_event`、`shijun/npc_mention` 三处槽位与默认文本，实现严格解析、代码级默认内容哈希、非阻塞 8 秒预取、章节作废和既有 StateEffect 原子通道；无线程保持 M1 节点行为等价，Unity EditMode 250/250。在线配置及真实 `/association/next` 端点保持关闭，未调用云服务，`MediaRefs` 目前仅校验而未进入表现层；本次批准内容改变 bundle 哈希，旧 v3 存档由既有兼容性门禁安全拒绝。真实 Provider、云预算及新审核文本仍须经过 2.4 节人工闸门 |
